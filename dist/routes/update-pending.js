...
var update_pending_exports = {};
__export(update_pending_exports, {
  updatePendingRoute: () => updatePendingRoute
});
module.exports = __toCommonJS(update_pending_exports);
var import_zod2 = require("zod");

// src/domain/entities/Pending.ts
var import_cuid2 = require("@paralleldrive/cuid2");
var Pending = class {
  constructor(id = (0, import_cuid2.createId)(), client2, callReason, status, priority, responsible, category, description, createdAt, updatedAt) {
    this.id = id;
    this.client = client2;
...
var DrizzlePendingRepository = class {
  ...
  async update(id, pendingData) {
    const response = await db.update(pendingIssues).set(pendingData).where((0, import_drizzle_orm.eq)(pendingIssues.id, id)).returning();
    return response[0] ?? null;
  }
  async delete(id) {
    const response = await db.delete(pendingIssues).where((0, import_drizzle_orm.eq)(pendingIssues.id, id)).returning();
  ...
  async selectOnePending(id) {
    const response = await db.select({
      ...
      createdAt: pendingIssues.createdAt,
      updatedAt: pendingIssues.updatedAt
    }).from(pendingIssues).where((0, import_drizzle_orm.eq)(pendingIssues.id, id));
    return response[0] ?? null;
  }
};

// src/routes/update-pending.ts
var updatePendingRoute = async (app) => {
  app.put(
    "/pendings/:id",
    {
      ...
      schema: {
        ...
        body: import_zod2.z.object({
          client: import_zod2.z.string().nullable().optional(),
          callReason: import_zod2.z.string().nullable().optional(),
          status: import_zod2.z.enum(["Aberto", "Encaminhado", "Pendente", "Conclu\xEDdo"]).nullable().optional(),
          priority: import_zod2.z.string().nullable().optional(),
          responsible: import_zod2.z.string().nullable().optional(),
