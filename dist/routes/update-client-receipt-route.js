...
var update_client_receipt_route_exports = {};
__export(update_client_receipt_route_exports, {
  updateClientReceiptRoute: () => updateClientReceiptRoute
});
module.exports = __toCommonJS(update_client_receipt_route_exports);
...
var ClientReceipt = class {
  constructor(id = (0, import_cuid22.createId)(), receiptDate, competence, cnpj, clientName, percentage, compensationMonth, honorary, tax, status) {
    this.id = id;
...
var DrizzleClientReceiptRepository = class {
  ...
  async update(id, data) {
    const response = await db.update(clientReceipt).set(data).where((0, import_drizzle_orm.eq)(clientReceipt.id, id)).returning();
    return response[0];
  }
  async delete(id) {
    const response = await db.delete(clientReceipt).where((0, import_drizzle_orm.eq)(clientReceipt.id, id)).returning();
    return response.length > 0;
  }
};

// src/routes/update-client-receipt-route.ts
var updateClientReceiptRoute = async (app) => {
  app.put(
    "/client-receipt/:id",
    {
      ...
      schema: {
        ...
        body: import_zod2.z.object({
          receiptDate: import_zod2.z.string().nullable(),
          competence: import_zod2.z.string().nullable(),
          cnpj: import_zod2.z.string().nullable(),
          clientName: import_zod2.z.string().nullable(),
          percentage: import_zod2.z.number().nullable(),
          compensationMonth: import_zod2.z.string().nullable(),
