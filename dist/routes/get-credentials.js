...
// src/routes/get-credentials.ts
var get_credentials_exports = {};
__export(get_credentials_exports, {
  getCredentialRoute: () => getCredentialRoute
});
module.exports = __toCommonJS(get_credentials_exports);
...
// src/routes/get-credentials.ts
var getCredentialRoute = async (app) => {
  app.get(
    "/credential",
    {
    ...
    async (_, reply) => {
      ...
      const credentials2 = await repo.select();
      return reply.status(200).send(credentials2);
    }
  );
};
