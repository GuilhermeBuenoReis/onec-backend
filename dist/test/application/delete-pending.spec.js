...
// src/test/application/delete-pending.spec.ts
describe("Delete Pending", () => {
  ...
  it("should be possible to delete the pending", async () => {
    const pendingData = {
      client: "Jhon Doe",
      callReason: "Problema t\xE9cnico",
      status: "Aberto",
      priority: "M\xE9dia",
      responsible: "Equipe de Suporte",
      category: "Atendimento",
      description: "O cliente relatou um problema t\xE9cnico que precisa ser resolvido.",
    ...
    const found2 = pendings.find((p3) => p3.id === "1");
  ...
  it("should throw an error when trying to delete a pending without an id", async () => {
    await globalExpect(repository.delete("")).rejects.toThrowError(
      "N\xE3o foi encontrado o chamado para ser deletado!"
    );
  });
});
