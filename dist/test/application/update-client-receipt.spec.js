...
var InMemoryClientReceiptRepository = class {
  ...
  async update(id, data) {
    const index2 = this.clientReceipts.findIndex(
      (clientReceipt) => clientReceipt.id === id
    );
    if (index2 === -1) return null;
    const updatedClientReceipt = { ...this.clientReceipts[index2], ...data };
    this.clientReceipts[index2] = updatedClientReceipt;
  ...
  async delete(id) {
    ...
    this.clientReceipts.splice(index2, 1);
    return true;
  }
};

// src/test/application/update-client-receipt.spec.ts
describe("update client", () => {
  ...
  it("shoud be able update a client", async () => {
    ...
    if (response) {
      globalExpect(response.clientName).toEqual(updatedClientData.clientName);
    }
  });
});
