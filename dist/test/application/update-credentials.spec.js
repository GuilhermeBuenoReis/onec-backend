...
var InMemoryCredentialRepository = class {
  ...
  async update(id, data) {
    const index2 = this.credentials.findIndex(
      (credential) => credential.id === id
    );
    if (index2 === -1) return null;
    const updatedCredential = { ...this.credentials[index2], ...data };
    this.credentials[index2] = updatedCredential;
  ...
  async delete(id) {
    ...
    this.credentials.splice(index2, 1);
    return true;
  }
};

// src/test/application/update-credentials.spec.ts
describe("updated credentials", () => {
  ...
  it("shoud be able update a new credentials", async () => {
    ...
    if (response) {
      globalExpect(response.channelHead).toEqual(updatedCredentialData.channelHead);
    }
  });
});
