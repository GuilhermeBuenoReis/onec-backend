...
var InMemoryUserRepository = class {
  ...
  async update(id, data) {
    const index2 = this.users.findIndex((u3) => u3.id === id);
    if (index2 === -1) return null;
    const updatedUser = { ...this.users[index2], ...data };
    this.users[index2] = updatedUser;
    return updatedUser;
  }
  async delete(id) {
    const index2 = this.users.findIndex((u3) => u3.id === id);
  ...
  async findByEmail(email) {
    return this.users.find((u3) => u3.email === email) || null;
  }
  async addUser(user) {
    this.users.push(user);
  }
...
// src/test/application/update-user.spec.ts
describe("Update User", () => {
  it("should update an existing user", async () => {
    ...
    if (!createdUser) {
      throw new Error("Falha na cria\xE7\xE3o do usu\xE1rio");
    }
    const updateData = {
    ...
    globalExpect(updatedUser).toBeDefined();
    globalExpect(updatedUser?.email).toBe(updateData.email);
    globalExpect(updatedUser?.role).toBe(updateData.role);
    globalExpect(updatedUser?.passwordHash).toBe(userData.passwordHash);
  });
});
