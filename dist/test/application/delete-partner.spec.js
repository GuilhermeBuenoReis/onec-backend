...
var InMemoryPartnerRepository = class {
  ...
  async update(id, partner) {
    ...
    const index2 = this.partners.findIndex((p3) => p3.id === id);
    if (index2 === -1) {
      return null;
    }
    const updatedPartner = { ...this.partners[index2], ...partner };
    this.partners[index2] = updatedPartner;
  ...
  async delete(id) {
    ...
    this.partners.splice(index2, 1);
    return true;
  }
};

// src/test/application/delete-partner.spec.ts
describe("Delete Partner", () => {
  ...
  it("should be possible to delete the partner", async () => {
    ...
    globalExpect(deletionResult).toBe(true);
    const partners = await repository.select();
    const found2 = partners.find((p3) => p3.id === "1");
    globalExpect(found2).toBeUndefined();
  });
  it("should return false when trying to delete a non-existent partner", async () => {
    ...
    globalExpect(deletionResult).toBe(false);
  });
  it("should throw an error when trying to delete a partner without an id", async () => {
    await globalExpect(repository.delete("")).rejects.toThrowError(
