...
// src/domain/repositories/memory/InMemoryClientRepostory.ts
var InMemoryClientRepository = class {
  ...
  async update(id, data) {
    const index2 = this.clients.findIndex((client) => client.id === id);
    if (index2 === -1) return null;
    const updatedClient = { ...this.clients[index2], ...data };
    this.clients[index2] = updatedClient;
    return updatedClient;
  }
  async delete(id) {
    const index2 = this.clients.findIndex((client) => client.id === id);
    if (index2 === -1) return false;
    this.clients.splice(index2, 1);
    return true;
  }
};

// src/test/application/update-client.spec.ts
describe("update client", () => {
  ...
  it("shoud be able update a client", async () => {
    ...
    globalExpect(response).not.toBeNull();
    if (response) {
      globalExpect(response.enterprise).toEqual(updatedClientData.enterprise);
    }
  });
});
