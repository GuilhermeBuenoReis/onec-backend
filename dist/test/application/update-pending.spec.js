...
// src/test/application/update-pending.spec.ts
describe("Update Pending", () => {
  ...
  it("should be possible to update the pending", async () => {
    ...
    if (result) {
      globalExpect(result.client).toBe("Updated Pending");
      globalExpect(result.priority).toBe("Alta");
    }
  });
  ...
  it("should throw an error when trying to update a pending without an id", async () => {
    ...
    ).rejects.toThrowError("N\xE3o foi encontrado o chamado para atualizar!");
  });
});
