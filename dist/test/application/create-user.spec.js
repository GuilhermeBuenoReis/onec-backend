...
// src/test/application/create-user.spec.ts
describe("Create User", () => {
  it("should create a new user with valid data", async () => {
    ...
    if (!createdUser) {
      throw new Error("Usu\xE1rio n\xE3o foi criado");
    }
    globalExpect(createdUser).toMatchObject(userData);
    globalExpect(createdUser.id).toBeDefined();
  });
  it("should throw an error when user creation fails", async () => {
    ...
    ).rejects.toThrow("Erro ao criar usu\xE1rio!");
  });
});
